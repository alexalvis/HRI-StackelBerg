...
def Transition(S, A, B, F, Product_A, p_H, p_L):
...
def initial_state(S):
    initlist = []
    c = np.zeros(len(S))
    initlist.append((0, 0, 0, 0))
    initlist.append((0, 0, 0, 1))
    initlist.append((0, 0, 0, 2))
    initlist.append((0, 0, 1, 1))
    initlist.append((0, 0, 1, 2))
    initlist.append((0, 0, 1, 3))
    for st in initlist:
        c[S.index(st)] = 1/len(initlist)
    return c
        ...
        for s in S:
            ...
            if s_index not in F:
                for act in pi_1[s].keys():
                    a = act
                Pro = P[s_index][(a, b)]
                V1[s_index] = R1[s_index][a][b] + gamma * sum(Pro[s_] * V1_[s_] for s_ in Pro.keys())
                V2[s_index] = R2[s_index][a][b] + gamma * sum(Pro[s_] * V2_[s_] for s_ in Pro.keys())
...
if __name__ == "__main__":
    ...
    p_H = 0.7
    p_L = 0.9
    P = Transition(S, A, B, F, Product_A, p_H, p_L)
    testP(P)
    Cost_A, Cost_B = Cost()
...
#    
    ...
    filename_policy = "policy_small.pkl"
    file = open(filename_policy, "wb")
    pickle.dump(policy, file)
    file.close()
    
    filename_V1 = "V1_small.pkl"
    file = open(filename_V1, "wb")
    pickle.dump(V1, file)
    file.close()
    
    filename_V2 = "V2_small.pkl"
    file = open(filename_V2, "wb")
    pickle.dump(V2, file)
    file.close()
