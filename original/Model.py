            ...
            if a in Available_A(s):
                ...
                for b in B:
                    ...
                    if b in available_B:
                        ...
                        P[s_index][(a_index,b_index)][len(S) - 1] = 1.0
            else:
                for b in B:
                    b_index = B.index(b)
                    P[s_index][(a_index,b_index)] = {}
...
def valueiteration(S, P, F, Sink, A, R1):
    gamma = 0.85
    policy = {s : {} for s in S}
    V1 = {}
    for s in S:
        s_index = S.index(s)
        if s_index not in F and s_index not in Sink:
            V1[s_index] = 0
        elif S.index(s) in F:
            V1[s_index] = 2000
        else:
...
def policyevaluation(S, pi_1, F, Sink):
    gamma = 0.85
    V1 = {}
    V2 = {}
    for s in S:
...
if __name__ == "__main__":
    ...
    V, policy = valueiteration(S, P, F, Sink, A, R1)
#    V1, V2 = policyevaluation(S, policy, F, Sink)
#    filename_S = "S.pkl"
...
#    file.close()
#    
#    filename_Sink = "Sink.pkl"
#    file = open(filename_Sink, "wb")
#    pickle.dump(Sink, file)
#    file.close()
#    
#    filename_R1 = "R1.pkl"
